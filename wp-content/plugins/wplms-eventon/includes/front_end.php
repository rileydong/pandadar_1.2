...
class Wplms_EventOn_Front_End{
	...
	function event_creation($tabs){	
		if(class_exists('WPLMS_Front_End_Fields')){
			$front_end = WPLMS_Front_End_Fields::init();	
			if(empty($front_end->course_id)){
				//return $tabs;
			}
		}
				...
				$event_args = array(
					...
					if($events->have_posts()){
						...
						while($events->have_posts()){
							...
							echo '<li><strong class="title" data-id="'.get_the_ID().'"><i class="icon-pin-2"></i> [ '.date_i18n( get_option( 'date_format' ), $start ).' ] '.get_the_title().'</strong>
	                                <ul class="data_links">
	...
	function save_course_events(){
        ...
        if(is_array($events) && isset($events)){
        	foreach($events as $event){
	            ...
	            print_r($event->id.'wplms_ev_course'.$course_id);
