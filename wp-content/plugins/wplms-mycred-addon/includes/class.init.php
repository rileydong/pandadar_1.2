...
class wplms_points_init {
		...
		$mycred_metabox = apply_filters('wplms_mycred_metabox',array(  
		    ...
		    $prefix.'mycred_subscription' => array( // Text Input
		     ...
		     $prefix.'mycred_duration_parameter'=>array( // Text Input
				'label'	=> __('Points duration parameter','vibe-customtypes'), // <label>
				'desc'	=> __('Subscription duration parameter','vibe-customtypes'), // description
	...
	function front_end($settings){
		if(!is_admin()){
			...
			$settings['vibe_mycred_subscription']['options'] = array('H'=>__('DISABLE','wplms-front-end' ),'S'=>__('ENABLE','wplms-front-end' ));	
		}
		return $settings;
	}
