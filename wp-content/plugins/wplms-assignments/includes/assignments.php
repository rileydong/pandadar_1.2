...
if (!class_exists('WPLMS_Assignments')){
    class WPLMS_Assignments
    {
        ...
        function get_assignment_submission_count($course_id){
            ...
            $count = $wpdb->get_var("SELECT count(*) FROM {$wpdb->postmeta} WHERE meta_value LIKE '0' AND post_id IN ($assignment_ids)");
            ...
            if(!empty($assignments)){
                ...
                <div class="submissions_form">
                    ...
                    foreach($assignments as $assignments_id){
                        ?>
                        <option value="<?php echo $assignments_id->post_id; ?>"><?php echo get_the_title($assignments_id->post_id); ?></option>
                        <?php   
                    }
                    ?>
                    </select>
        ...
        function fetch_assignment_submissions(){
            ...
            global $wpdb;
            if($_POST['status'] == 1){
                $assignment_submissions = $wpdb->get_results($wpdb->prepare ("SELECT c.user_id as user_id,p.post_id as assignment_id FROM {$wpdb->postmeta} as p LEFT JOIN {$wpdb->comments} as c ON p.post_id = c.comment_post_ID WHERE CAST(c.user_id as UNSIGNED) = CAST(p.meta_key as UNSIGNED) AND CAST(p.meta_value as UNSIGNED) != 0 && p.post_id = %d LIMIT 0,999",$assignment_id), ARRAY_A);    
            }else{
                $assignment_submissions = $wpdb->get_results($wpdb->prepare ("SELECT c.user_id as user_id,c.comment_post_ID as assignment_id FROM {$wpdb->comments} as c WHERE c.comment_post_ID = %d AND NOT EXISTS (SELECT * FROM {$wpdb->postmeta} as p WHERE p.post_id = %d AND p.meta_value > '0' AND p.meta_key = c.user_id ) LIMIT 0,999",$assignment_id,$assignment_id), ARRAY_A);  
            }
