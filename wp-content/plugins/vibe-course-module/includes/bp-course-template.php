		...
		function bp_course_is_member($course_id = null, $user_id = null){
			...
			if(!is_user_logged_in())
		  	...
		  	if(empty($course_id)){
		   	...
		   	$instructor_id = apply_filters('wplms_course_instructors',$instructor_id,$course_id);
			...
			foreach($course_items as $key => $item){
				if(is_numeric($item)){
					...
					$duration_parameter = apply_filters("vibe_".$type."_duration_parameter",60,$item);
				 	...
				 	$duration = '<span class="time"><i class="fa fa-clock-o"></i> '.(($duration >9998)?_x('Unlimited','Unlimited unit duration label','vibe'):gmdate("H:i:s", $duration*$duration_parameter)).'</span>';
...
function bp_course_get_max_students($course_id = NULL){
	if(empty($course_id)){
		$course_id = get_the_ID();
	}
	$max_students = apply_filters('bp_course_get_max_students','',$course_id);
...
function bp_course_get_start_date($course_id = NULL){
	if(empty($course_id)){
		$course_id = get_the_ID();
	}
	$start_date = apply_filters('bp_course_get_start_date','',$course_id);
	if(empty($start_date)){
...
function bp_course_get_course_duration($course_id = NULL){

	if(empty($course_id)){
		$course_id = get_the_ID();
	}
	$duration = apply_filters('bp_course_get_course_duration','',$course_id);
	if(empty($duration)){
		$set_duration = get_post_meta($course_id,'vibe_start_date',true);
