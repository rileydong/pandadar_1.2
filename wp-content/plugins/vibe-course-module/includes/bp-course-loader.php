...
function wplms_check_plugin_notice() {
	...
	if(!empty($parent) && defined('THEME_SHORT_NAME') && $wplms->name != 'WPLMS Blank ChildTheme'){
		...
		if(in_array($wplms->name,array('WPLMS Modern','WPLMS Instructor'))){			
			...
			if (version_compare($val, "2.0.6") < 0) {
				echo '<div class="error">
				    <p>'.sprintf(__( 'Please Update the %s theme to latest version %s', 'vibe' ),$wplms_parent->name,'2.0.6').'</p>
				  </div>';
			}

			if (version_compare($value, "2.0.6") < 0) {
				echo '<div class="error">
				    <p>'.sprintf(__( 'Please Update the %s theme to latest version %s', 'vibe' ),$wplms->name,'2.0.6').'</p>
				  </div>';
			}
		}
