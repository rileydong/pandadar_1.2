...
$('.quiz_results_popup').magnificPopup({
    ...
    callbacks: {
             parseAjax: function( mfpResponse ) {
              mfpResponse.data = $(mfpResponse.data).find('.quiz_result');
            },
...
function bp_course_generate_cookie(){
  var category_filter=[];
     ...
     $('.course_cat_nav li.current-cat').each(function(){
        if(typeof $(this).attr('data-slug') != 'undefined'){
          var category={'type':'course-cat','value':$(this).attr('data-slug')};
          category_filter.push(category);
        }
     });
...
jQuery(document).ready(function($){
   ...
   jQuery('.course_timeline.accordion').each(function(){ 
      ...
      prevSections = prevSections.add(prevSections.eq(0).prev());
      prevSections.trigger('click');
   });
