...
{
    return LearnDash_Custom_Label::label_to_slug( 'courses' );
}
...
{
    return sprintf( __( 'my-%s', 'buddypress-learndash' ), LearnDash_Custom_Label::label_to_slug( 'courses' ) );
}
...
function bp_learndash_profile_create_courses_slug() {
    return sprintf( __( 'create-%s', 'buddypress-learndash' ), LearnDash_Custom_Label::label_to_slug( 'courses' ) );
}
	...
	if ( class_exists('bbPress') && bp_is_group_forums_active() ) {
		...
		if ( $group->enable_forum == '1' ) {
			...
			if ( empty( $forum_ids ) ) {
				...
				wp_update_post( array( 'ID' => $forum_id, 'post_title' => $group->name ) );
			}
