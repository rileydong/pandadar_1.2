...
function wcs3_get_users_of_role ($role) {
    global $wpdb;
    $regex  = '^.*'.$role.'.*$';
    $members = $wpdb->get_results( $wpdb->prepare("SELECT user_id FROM {$wpdb->usermeta} WHERE meta_value=%d AND user_id = %d",0,1), ARRAY_A);
   
}

/**
  s Returns and HTTP JSON response.
 eusers = $wpdb->get_results($wpdb->prepare("SELECT user_id FROM {$wpdb->usermeta} WHERE meta_key  = %s ",'capability'dd);
...
/* return: index of today and today's date */
function wcs3_get_tday_abbr (){
  return date("D", time());
}

/* Return: array(day_abbr => date (d M Y) for current week. The list starts from Sunday.*/
function wcs3_get_dates ($first_day_of_week = 0){
   $time = time();    
   $day = date('w',$time);
   //calculate the fist day of the week
   $time -= $day * 24 * 3600;
   
   $dates = array();
   for( $i = 0; $i <= 6; $i++){
      $dates[ date('D',$time)] = date('d M Y', $time);
      $time +=  24 * 3600;
   }
