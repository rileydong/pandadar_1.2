...
function wcs3_add_or_update_schedule_entry_callback() {
    ...
    /*local time str*/
    $start_here = $start_hour . ':' . $start_minute . ':00';
    $end_here = $end_hour . ':' . $end_minute . ':00';
    ...
    $start_dt = new DateTime( WCS3_BASE_DATE . ' ' . $start_here, $tz );
    $start_dt->setTimeZone(new DateTimeZone('UTC')); 

    $end_dt = new DateTime( WCS3_BASE_DATE . ' ' . $end_here, $tz );
    $end_dt->setTimeZone(new DateTimeZone('UTC'));

    // UTC time str to put in database
    $start = $start_dt->format('H:i').':00';
    $end = $end_dt->format('H:i').':00';
    $utc_day = $start_dt->format('Y-m-d');
/* TODO: need to fix date change caused by timezone change.
    if ($utc_day > WCS3_BASE_DATE){
           $weekday +=1;
    }
    elseif ($utc_day < WCS3_BASE_DATE){
           $weekday -=1;
    }
  */  
