...
class WPLMS_tips{
			...
			foreach($this->settings as $key=>$setting){
				switch($key){
					...
					case 'course_curriculum_unit_description':
						...
						add_filter('wplms_curriculum_course_lesson',array($this,'unit_expander'));
					break;
					...
					case 'unit_quiz_start_datetime':
						...
						add_filter('wplms_drip_value',array($this,'apply_unit_date_time_drip_feed'),99,4);
					break;
    ...
    */
	function show_terms_conditions(){
        ...
        $terms_link = get_post_permalink( $page_id, false, false );
         echo '<input type="checkbox" name="terms_conditions" id="bph_field" value="1" /> I agree to these <a href="'.$terms_link.'">'.__('Terms and Conditions','vibe-customtypes').'</a>.';    
	}
    /*
		...
		if(isset($assignments) && is_array($assignments) && count($assignments)){
			foreach($assignments as $assignment){
				...
				<tr class="course_lesson">
					...
					<td><?php echo ($duration > 9998 ? _x('UNLIMITED','vibe-customtypes','assignment duration in curriculum') : tofriendlytime(($duration*$assignment_duration_parameter))); ?></td>
				</tr>
				<?php
			}
		}
            ...
            $link = trailingslashit( bp_loggedin_user_domain() . get_post_field('post_name',$myaccount_page_id) );
            	...
            	if ( isset($myaccount_page_id) && is_numeric($myaccount_page_id) ) {
	              $loggedin_menu['orders']=array(
	                          ...
	                          'link' => "https://pandadar.com/wordpress/wooaccount/"//$link
	                          );
	            }
	        }
