...
if(isset($_GET['action']) && $_GET['action']){ // Check Action
	...
	switch($post_type){
		...
		case 'quiz':
			...
			$retakes=apply_filters('wplms_quiz_retake_count',get_post_meta($quiz_id,'vibe_quiz_retakes',true),$quiz,$course,$user_id);
			...
			$sum=$total_sum=0;
			echo '<div class="quiz_result"><h3 class="heading">'.get_the_title($quiz_id).((isset($course) && is_numeric($course))?'<a href="'.get_permalink($course).'" class="small_link">( &larr; '.__('BACK TO COURSE','vibe').' )</a>':'');
			...
			echo' <a class="print_results"><i class="icon-printer-1"></i></a></span></h3>';
			if(count($questions)){
