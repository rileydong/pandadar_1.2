...
class WPLMS_Actions{
    ...
    function transparent_header_title_background(){ 
    	...
    	if($header_style == 'transparent'){ 
	    	if(is_page() || is_single() || bp_is_directory() || bp_current_component() || is_archive()){ 
	    		global $post;
				...
				if(!empty($title_bg)){
	    		?>
	    		<style>.course_header,.group_header{background:url(<?php echo $title_bg; ?>) !important;}#title{background:url(<?php echo $title_bg; ?>) !important;padding-bottom:30px !important; background-size: cover;}#title h1,#title h5,#title a,#title,#title #item-admins h3,body:not(.c4) .breadcrumbs li+li:before,.group_header div#item-header-content,.group_header #item-header-content h3 a,.bbpress .bbp-breadcrumb .bbp-breadcrumb-sep:after{color:#fff !important;}.group_header #item-header{background-color:transparent !important;}</style>
	    		<?php
	    		}
	    	}
    	}
...
*/
			...
			bp_core_new_subnav_item( array(
				...
				'slug'            => $slug,
				'parent_slug'     => $slug,
				'parent_url'      => "https://pandadar.com/wordpress/members-2/wooaccount/", //$link
				'position'        => 10,
			...
			foreach($endpoints as $key => $endpoint){
				...
				bp_core_new_subnav_item( array(
					...
					'slug'            => $key,
					'parent_slug'     => $slug,
					'parent_url'      => "https://pandadar.com/wordpress/wooaccount/",//$link,
					'position'        => $i,
					'item_css_id'     => 'nav-' . $key,
